...
// Saves 'buffer' to 'file_path'. The 'format' can be specified or it will be
// deduced from 'file_path' extension.
// If 'file_path' is "-", data is saved to stdout.
...
WP2Status Write16bAsPGM(const ArgbBuffer& buffer, FILE* fout);
...
WP2Status CompressWebP(const ArgbBuffer& buffer, const WebPConfig& config,
                       Writer* const output);
#endif
// Same as above but maps wp2 'quality' and 'speed' to a WebPConfig.
