...
void BM_EncodeImage(benchmark::State& state) {
  ...
  for (const auto& _ : state) {
    ...
    MemoryWriter data;
    EXPECT_EQ(Encode(original, &data), WP2_STATUS_OK);
  }
}
BENCHMARK(BM_EncodeImage)->DenseRange(0, testing::kNumFiles - 1);
