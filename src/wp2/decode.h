...
struct BitstreamFeatures {
 public:
  ...
  Orientation orientation;   // Rotation applied during decoding.
  ...
  RGB12b preview_color{0xFu, 0xFu, 0xFu};  // Average color in RGB444 format.
  uint32_t quality_hint;     // Encoding quality setting.

  bool has_icc;              // Indicates the presence of ICC information.
  bool has_trailing_data;    // Trailing data is present (XMP, EXIF, ...).
  TransferFunction transfer_function;

  uint8_t loop_count;        // Number of loops for animation (0 = infinite).
...
// Copies metadata to 'xmp', 'iccp' and 'exif' (if not null) from a whole WP2
// bitstream stored in 'data', in case only metadata is wanted. Otherwise it's
// available in ArgbBuffer after a regular Decode().
// Clears any previous data. Returns WP2_STATUS_OK or an error.
WP2_EXTERN WP2_NO_DISCARD WP2Status ExtractMetadata(const uint8_t* data,
                                                    size_t data_size,
                                                    Data* exif, Data* iccp,
                                                    Data* xmp);
...
class Decoder {
 public:
  ...
  // If so, 'duration_ms' can be retrieved if not null (0 for a still image).
  ...
  // These functions return the image features or nullptr if not yet available.
  ...
  // Returns a lower bound of the number of frames that were fully decodable
  // with the given bitstream so far. Never decreases (even rewind).
  // Does not decode anything; use SkipNumNextFrames(kMaxNumFrames) then
  // ReadFrame() prior to calling this to get the final count without
  ...
  // Disables pixel decoding for the next 'num_frames_to_skip'
  // (GetCurrentFrameIndex() included), but still reads frame features.
  // Useful for going directly to a timestamp (it will decode the previous
  // frames if required but ReadFrame() will return false for these) or
  // getting frames info without decoding ('kMaxNumFrames' to skip everything).
  ...
  // If 'output_buffer_changed' is false, some decoding might be avoided.
  void Rewind(bool output_buffer_changed = true);
 ...
 private:
  ...
  WP2Status BlendFrame(Rectangle window, bool dispose);
...
class CustomDecoder : public Decoder {
 ...
 private:
  ...
  // be at most 'num_requested_bytes' if possible.
