...
class PartialTileDecoder : public WP2Allocable {
 public:
  ...
  virtual WP2Status Init(const BitstreamFeatures& features,
                         ...
                         Tile* const partial_tile) = 0;
  // Starts work. Returns WP2_STATUS_OK or an error.
  virtual WP2Status Start() = 0;
  // Continues work. Returns WP2_STATUS_OK, WP2_STATUS_NOT_ENOUGH_DATA or error.
...
struct Decoder::State : public WP2Allocable {
 protected:
  explicit State(DataSource* const input) : data_source(input) {}
 ...
 public:
  ...
  // What data is expected next; the order matters.
  enum Stage {
    WAITING_FOR_HEADER_CHUNK = 0,
  ...
  // Number of frames that were (at least once) fully available in the stream.
  uint32_t num_available_final_frames = 0;
  ...
  ArgbBuffer raw_output_buffer = ArgbBuffer(WP2_Argb_32);
  ...
  ArgbBuffer frame_output_buffer = ArgbBuffer(WP2_Argb_32);
  // Temporary buffer used for blending when raw_output_buffer is already used.
  ArgbBuffer blended_buffer = ArgbBuffer(WP2_Argb_32);
  ...
  // Index of the frame whose pixels are currently stored in the output buffer.
  // If partially decoded, it is set to 'kMaxNumFrames' instead.
  uint32_t index_of_frame_in_output_buffer = kMaxNumFrames;
  ...
  // Used to decode parts of a tile, if not null. Must be destroyed first.
  ...
  // features are remembered. Pixels are kept unless 'output_buffer_changed'.
  virtual void Rewind(bool output_buffer_changed);
};

struct DecoderStateArray : public Decoder::State {
 public:
  DecoderStateArray() : Decoder::State(&array_data_source) {}

  // Rewinds also the 'array_data_source' so no need to call SetInput() again.
  void Rewind(bool output_buffer_changed) override;
...
struct DecoderStateStream : public Decoder::State {
 public:
  DecoderStateStream() : Decoder::State(&stream_data_source) {}
...
struct DecoderStateCustom : public Decoder::State {
 public:
  explicit DecoderStateCustom(CustomDecoder* const custom_decoder)
      : Decoder::State(&custom_data_source),
